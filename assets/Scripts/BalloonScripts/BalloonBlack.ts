...
export class BalloonBlack extends Component {
    ...
    onLoad() {
        ...
        if (!this.animation) {
            console.error("Animation not found into BalloonBlack");
        }

        this.audioSource = this.getComponent(AudioSource);

        if (!this.audioSource) {
            console.error("AudioSource not found into BalloonBlack");
        }
    }
    ...
    onPopAnimationFinished(){
        if (this.game) this.game.endGame(1);
        this.node.destroy();
    }
